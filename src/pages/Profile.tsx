...
// Updated style constants using our new color scheme
const neuBackground = "bg-[#F8F9FA]"; // Near-White
const neuInset = "bg-[#F8F9FA] shadow-[inset_3px_3px_6px_#d8dde2,inset_-3px_-3px_6px_#ffffff]";
const neuElevated = "bg-white shadow-[3px_3px_10px_#d1d9e6,-3px_-3px_10px_#ffffff]";
const neuAzure = "bg-[#e5f4fa]"; // Light Azure background
const neuAzureElevated = "bg-[#e5f4fa] shadow-[3px_3px_10px_#c3d0d8,-3px_-3px_10px_#ffffff]";
const neuAzureInset = "bg-[#e5f4fa] shadow-[inset_3px_3px_6px_#c3d0d8,inset_-3px_-3px_6px_#ffffff]";
...
const WritePrompt = ({ 
  ...
  <div className={`${neuElevated} ${neuRounded} p-5 mb-4 relative`}>
    ...
    <div className="border-t border-gray-100 pt-4">
      <Button 
        variant="secondary" 
        className={`w-full ${neuAzureElevated} hover:shadow-[4px_4px_8px_#c3d0d8,-4px_-4px_8px_#ffffff] active:${neuAzureInset} text-azure border-0 ${neuRounded} ${neuTransition}`}
      >
  ...
  return (
    <div className={`pb-20 ${neuBackground} min-h-screen`}>
      <div className="bg-white shadow-md p-4 flex items-center justify-between sticky top-0 z-10">
        <Button 
          variant="ghost" 
          className="text-azure font-medium p-2 hover:bg-[#e5f4fa] hover:text-azure flex items-center text-base"
          onClick={() => navigate('/')}
        >
          <ArrowLeft className="h-5 w-5 mr-1" />
          Back
        </Button>
        <h1 className="text-2xl font-bold text-darkgray">Lia</h1>
        <Button 
          variant="ghost" 
          className="text-azure font-medium p-2 hover:bg-[#e5f4fa] hover:text-azure flex items-center text-base"
      ...
      <Tabs defaultValue="edit" className="w-full">
        <div className="bg-white border-b border-gray-200 px-4 sticky top-[72px] z-10">
          <TabsList className="w-full grid grid-cols-2 bg-transparent h-14">
            <TabsTrigger 
              value="edit" 
              className={`h-full data-[state=active]:text-azure data-[state=active]:border-b-2 data-[state=active]:border-azure text-xl font-medium ${neuTransition}`}
            >
              Edit
            </TabsTrigger>
            <TabsTrigger 
              value="view" 
              className={`h-full data-[state=active]:text-azure data-[state=active]:border-b-2 data-[state=active]:border-azure text-xl font-medium text-coolgray ${neuTransition}`}
            >
              View
            </TabsTrigger>
          </TabsList>
        ...
        <TabsContent value="edit" className="px-5 pt-6 space-y-8 mt-0 h-full">
          ...
          <ProfileSection title="Written Prompts">
            ...
            <div className="p-6 border-2 border-dashed border-gray-200 rounded-xl flex items-center justify-between m-4">
              ...
              <Button 
                size="icon" 
                className={`${neuAzureElevated} rounded-full text-azure h-12 w-12 ${neuTransition}`}
              >
          ...
          <ProfileSection title="Poll (1)">
            <div className="p-6 border-2 border-dashed border-gray-200 rounded-xl flex items-center justify-between m-4">
              <div>
                <p className="text-xl font-medium mb-1 text-darkgray">Create a poll</p>
                <p className="text-base text-coolgray italic">And add your options</p>
              </div>
              <Button 
                size="icon" 
                className={`${neuAzureElevated} rounded-full text-azure h-12 w-12 ${neuTransition}`}
        ...
        <TabsContent value="view" className="mt-0">
          <div className={`flex justify-center items-center h-60 text-coolgray ${neuInset} mx-5 my-8 ${neuRounded}`}>
            <p className="text-base">Preview mode content</p>
        </div>
        </TabsContent>
      </Tabs>
    </div>
